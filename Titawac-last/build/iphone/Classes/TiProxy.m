...
//Rare exceptions to indicate a bug in the _titanium_wac code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_titanium_wac = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_titanium_wac]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.titanium_wac.module"
